...
export class ChatController {
  ...
  async getDelayMessages(
    ...
    return Promise.all(prismaMessages.map(async (message) => convertPrismaMessage(
      message,
      [],
      await chatService.getFileUrls(message.fileKeys),
      message.inTime,
    )));
  }
  ...
  async createGroup(
    ...
    const createManyUserChat = toUsers.map((user) => prisma.userChat.create({
  ...
  @Res() response: express.Response,
    ...
    const group = await prisma.chat.findUnique({ where: { joinKey } });
    if (group === null) {
