...
import {
  ChatType, MessageReactionType, MessageType, MetadataType, ReactionType, ReactionWithMessageInfoType,
} from '../types/types';

const messageForm = (chatId: number, text: string, files: File[], inTime?: Date): FormData => {
...
class UserApiClient extends BaseApiClient {
  sendMessage(chatId: number, text: string, files: File[]): Promise<MessageType> {
    return this.axiosPostForm('/chat/send', messageForm(chatId, text, files));
  }

  sendDelayMessage(chatId: number, text: string, files: File[], inTime: Date): Promise<MessageType> {
    return this.axiosPostForm('/chat/send/delay', messageForm(chatId, text, files, inTime));
  }
